...
<div class="form-container">
  ...
  <form class="form" method="POST" action="{{ route('tasks.update', ['id' => $task->id]) }}">
    ...
    <div class="form-item">
      <label>Detail:</label>
      <textarea class="form-text-area" value="{{ old('detail',$task->detail) }}" name="detail"></textarea>
    </div>
...
<div class="uploaded-files">
  ...
  <li class="uploaded-file">
    ...
    <form id="file-delete-form-{{ $file->id }}"
      action="{{ route('tasks.files.destroy', ['task_id' => $task->id, 'id' => $file->id]) }}" method="POST"
      style="display: none;">
      @csrf
      @method('delete')
    </form>
...
<div class="form-container">
  <form classs="form" method="POST" action="{{ route('tasks.files.store', ['task_id' => $task->id]) }}"
